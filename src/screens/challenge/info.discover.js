 ...
 * @constructor
 */
function ChallengeDiscoverDetailInfoScreen({ route, navigation }) {
  ...
  const onPressStartChallenge = () => {
    if (currentChallenge != null) {
      if (currentChallenge.challenge_detail._id !== challengeDetail._id) { //? the user is in another challenge
        ...
        navigation.navigate('_ChallengeDiscoverDetailStart', {
          key: '_ChallengeDiscoverDetailStart',
  ...
  const startChallengeNow = (mode, participants_id) => {
    ...
    userAPI.startChallenge(params).then((res) => {
      ...
      let screen = 'ChallengeDiscoverDetailStart'
      if (mode == 'team') screen = 'ChallengeDiscoverDetailStartTeam'
  ...
  return (
    <DefaultView>
      <Appbar.Header statusBarHeight={0}>
        ...
        <Appbar.Content title={challengeDetail.name} color={colors.primary} />
      </Appbar.Header>
      ...
      <View style={{ flex: 1, paddingHorizontal: 10 }}>
        ...
        <View style={{ flex: 0.6, paddingHorizontal: 10, marginTop: 20, alignItems: 'center' }}>
          <View style={{ flex: 0.35, justifyContent: 'center', marginTop: 5, marginBottom: 5 }}>
            {/* <Text style={{ textAlign: 'center', flex: 1 }}>Your challenge is to discover the</Text>
            <TextBold style={{ textAlign: 'center', flex: 1 }}>{challengeDetail.place_detail.local_name}</TextBold> */}
          ...
          <View style={{ flex: 1, justifyContent: 'center' }}>
            <Text style={{ textAlign: 'center' }}>Donations from this challenge will go to <TextBold>{challengeDetail.charity_detail.name}</TextBold> who <TextBold>{challengeDetail.charity_detail.description}</TextBold>.</Text>
          </View>
...
export default ChallengeDiscoverDetailInfoScreen
